            ...
            <Card className="border-red-200 bg-red-50/50 dark:bg-red-950/20">
              ...
              <CardContent className="space-y-2">
                ...
                {incident.breach_notification_deadline && (
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-muted-foreground">72h Deadline</span>
                    <span className={cn(incident.is_breach_notification_overdue ? "text-red-600 font-medium" : "")}>
                      {format(new Date(incident.breach_notification_deadline), "MMM d, yyyy HH:mm")}
                    </span>
                  </div>
                )}
                {incident.is_breach_notification_overdue && (
                  <div className="mt-2 rounded-md bg-red-100 dark:bg-red-900/30 px-3 py-2 text-xs text-red-700 dark:text-red-300">
                    <AlertTriangle className="inline h-3 w-3 mr-1" />
                    72-hour GDPR notification deadline has passed. Notify the supervisory authority immediately.
                  </div>
                )}
                {incident.breach_notification_deadline && !incident.is_breach_notification_overdue && !incident.gdpr_notification_sent_at && (
                  <div className="mt-2 rounded-md bg-yellow-50 dark:bg-yellow-900/20 px-3 py-2 text-xs text-yellow-700 dark:text-yellow-300">
                    <AlertTriangle className="inline h-3 w-3 mr-1" />
                    {(() => {
                      const remaining = differenceInHours(new Date(incident.breach_notification_deadline!), new Date());
                      return `${remaining}h remaining to notify the supervisory authority.`;
                    })()}
                  </div>
                )}
              </CardContent>
