...
class IncidentAdmin(admin.ModelAdmin):
    list_display = [
        "title", "severity", "status", "category", "owner", "reporter",
        "detected_at", "is_data_breach", "gdpr_notification_required",
    ]
    list_filter = ["status", "severity", "category", "is_data_breach", "gdpr_notification_required"]
    ...
    readonly_fields = ["id", "contained_at", "resolved_at", "closed_at", "created_at", "updated_at"]
    fieldsets = [
        (None, {"fields": ["id", "title", "description", "category", "severity", "status"]}),
        ("People", {"fields": ["owner", "reporter"]}),
        ("Timeline", {"fields": ["detected_at", "reported_at", "contained_at", "resolved_at", "closed_at"]}),
        ("Data & GDPR", {"fields": ["is_data_breach", "gdpr_notification_required", "gdpr_notification_sent_at"]}),
